...
def generate_patient_features(rhythm_type: str, n_samples: int = 200) -> np.ndarray:
    ...
    for _ in range(n_samples):
        ...
        if rhythm_type == "nsr":
            ...
            ef = np.random.normal(48.0, 8.0) # often reduced EF
            hr = np.random.normal(110.0, 20.0) # rapid rate
            irreg = np.random.uniform(0.18, 0.35) # highly irregular
            sdnn = np.random.normal(115.0, 20.0) # highly variable
            rmssd = np.random.normal(90.0, 18.0)
            pnn50 = np.random.normal(35.0, 8.0)
            lf = np.random.normal(150.0, 50.0) # reduced LF
            lf_hf = np.random.uniform(0.15, 0.50) # low LF/HF ratio
            hf = lf / lf_hf
            sd1 = np.random.normal(65.0, 12.0)
            sd2 = np.random.normal(85.0, 15.0) # fan-shaped (high SD1 relative to SD2)
            sd_ratio = sd1 / sd2
            dys = np.random.normal(115.0, 20.0) # dyssynchrony
            mot_irreg = np.random.normal(0.42, 0.08) # chaotic wall motion
            
        elif rhythm_type == "pvc":
            ef = np.random.normal(52.0, 6.0)
            hr = np.random.normal(74.0, 10.0)
            irreg = np.random.uniform(0.08, 0.14) # moderate global irregularity
            sdnn = np.random.normal(75.0, 15.0)
            rmssd = np.random.normal(70.0, 12.0) # RMSSD spike due to ectopic beats
            ...
            sd_ratio = sd1 / sd2
            dys = np.random.normal(130.0, 25.0) # high dyssynchrony during ectopic beats
            mot_irreg = np.random.normal(0.35, 0.06) # wall motion jerk
            
        elif rhythm_type == "bradycardia":
            ef = np.random.normal(56.0, 5.0)
            hr = np.random.normal(48.0, 5.0) # slow rate
            irreg = np.random.uniform(0.01, 0.06) # regular
            ...
            hr = np.random.normal(125.0, 12.0) # fast rate
            irreg = np.random.uniform(0.01, 0.06) # regular
            sdnn = np.random.normal(30.0, 8.0) # lower variability at high rates
...
def train():
    ...
    # 1. Fit standard scaler and extract parameters
    ...
    # Using Booster API to ensure compatibility with Booster.load_model in the agent
