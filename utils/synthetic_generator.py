...
def generate_ultrasound_frame(t: int, rx: float, ry: float, angle: float, noise_level: float = 20.0) -> np.ndarray:
    """
    Generates a single simulated ultrasound frame of a Left Ventricle.
    Features an ultrasound sector boundary, a bright myocardial wall, a dark ventricular cavity,
    ...
    # 1. Draw background ultrasound sector scan sector (cone shape)
    # Center of sector at top middle (56, 0)
    center = (56, -10)
    # Draw sector outline
    cv2.ellipse(frame, center, (110, 110), 90, -35, 35, 30, -1)
    
    # Add tissue background (dim gray with noise)
    ...
    # 2. Draw contracting LV Myocardial wall (bright gray ellipse contour)
    cx, cy = 56, 56
    # Rotated ellipse representing the LV wall (larger than cavity)
    wall_rx = rx + 3.0
    wall_ry = ry + 4.0
    
    wall_mask = np.zeros((h, w), dtype=np.uint8)
    cv2.ellipse(wall_mask, (cx, cy), (int(wall_rx), int(wall_ry)), int(angle), 0, 360, 255, -1)
    
    # Myocardial tissue texture (brighter, dense noise)
    myo_noise = np.random.normal(160, 25, (h, w)).astype(np.uint8)
    frame_indices = (wall_mask == 255) & (frame > 0)
    frame[frame_indices] = myo_noise[frame_indices]
    
    # 3. Draw LV Cavity (dark central ellipse)
    cavity_mask = np.zeros((h, w), dtype=np.uint8)
    cv2.ellipse(cavity_mask, (cx, cy), (int(rx), int(ry)), int(angle), 0, 360, 255, -1)
    
    # Cavity blood texture (very dark, fine-grained noise)
    blood_noise = np.random.normal(15, 6, (h, w)).astype(np.uint8)
    cavity_indices = (cavity_mask == 255) & (frame > 0)
    frame[cavity_indices] = blood_noise[cavity_indices]
    
    # 4. Apply a global Gaussian blur to simulate ultrasound resolution degradation
    frame = cv2.GaussianBlur(frame, (5, 5), 1.2)
    
    # 5. Add post-processed speckle noise
...
def generate_synthetic_video(rhythm: str, output_path: str, T: int = 150, fps: int = 50):
    ...
    print(f"Generating synthetic {rhythm.upper()} video ({T} frames, {fps} FPS) -> {output_path}...")
    ...
    for t in range(T):
        ...
        # Calculate contracting ellipse parameters based on rhythm profile
        if rhythm == "normal":
            ...
            # AFib: fluctuating instant heart rates and stroke volumes
            phase = 0.0
            for k in range(t + 1):
            ...
            # PVC: early ventricular beat with a weak contraction, followed by a long pause
            cycle_frame = t % 120
            if cycle_frame < 40: # Normal beat 1
                phase = 2 * np.pi * (72 / 60.0) * (cycle_frame / float(fps))
                vol_amp = 7.0
            elif cycle_frame < 70: # Early PVC beat
                pvc_t = (cycle_frame - 40)
                phase = 2 * np.pi * (110 / 60.0) * (pvc_t / float(fps)) + np.pi/2
                vol_amp = 3.5 # weak contraction
            else: # Compensatory pause
                phase = np.pi # diastolic state (relaxed)
                vol_amp = 1.0
        ...
        # Rotate ellipse slightly dynamically to simulate wall motion
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate synthetic Left Ventricle ultrasound videos.")
    ...
    parser.add_argument("--output", type=str, default="", help="Output video path (default: data/synthetic_<rhythm>.avi)")
    parser.add_argument("--frames", type=int, default=150, help="Number of frames to generate (default: 150)")
    parser.add_argument("--fps", type=int, default=50, help="Video frame rate (default: 50)")
